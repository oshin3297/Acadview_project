...
client = MongoClient("mongodb://test:test123@ds215633.mlab.com:15633/web_app_quiz")
db = client.get_database('web_app_quiz')
user_record= db.user_record
...
def index():
	...
	if request.method == 'POST':
		search = user_record.find_one({'username':form.username.data})
		if search:
			msg = 'Username already exists'
		else:
			user_record.insert({'username':form.username.data,'password':form.password.data})
...
def result():
	...
	msg = ''
	if count<4:
		msg="Poor result...Need to work hard"
	elif count>8:
